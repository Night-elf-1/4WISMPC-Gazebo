...
class FwisNonlinearMpcControllerNode
{
public:
    FwisNonlinearMpcControllerNode(ros::NodeHandle& nh, ros::NodeHandle& pnh)
          ...
          tf_listener_(tf_buffer_)
    {
        ...
        // 订阅参考路径、曲率、速度和里程计
        path_sub_ = nh.subscribe("/reference_path", 1, &FwisNonlinearMpcControllerNode::pathCallback, this);
        curvature_sub_ = nh.subscribe("/reference_curvature", 1, &FwisNonlinearMpcControllerNode::curvatureCallback, this);
        speed_sub_ = nh.subscribe("/reference_speeds", 1, &FwisNonlinearMpcControllerNode::speedCallback, this);
        odom_sub_ = nh.subscribe("/odom", 1, &FwisNonlinearMpcControllerNode::odomCallback, this);
        ...
        // 调试：发布最近参考点
        nearest_point_pub_ = nh.advertise<visualization_msgs::Marker>("/mpc_nearest_point_nmpc", 1);

        // 控制定时器
        timer_ = nh.createTimer(ros::Duration(1.0 / control_rate_), &FwisNonlinearMpcControllerNode::controlLoop, this);

        ROS_INFO("FWIS Nonlinear MPC controller initialized. wheel_radius=%.3f, control_rate=%.1f Hz",
...
private:
    ...
    void controlLoop(const ros::TimerEvent&)
    {
        ...
        ROS_INFO_THROTTLE(1.0, "NMPC: nearest_idx=%d, nearest_dist=%.3f, solve_time=%.2fms, v=[%.2f, %.2f, %.2f, %.2f]",
                          min_index, min_e, solve_time_ms,
                          U_solve(0), U_solve(1), U_solve(2), U_solve(3));
        ROS_INFO_THROTTLE(1.0, "NMPC: steering=[%.3f, %.3f, %.3f, %.3f]",
...
private:
    parameters param_;
    fwisNonlinearMpcController mpc_;
...
{
    ros::init(argc, argv, "fwis_nonlinear_mpc_controller_node");
    ros::NodeHandle nh;
    ros::NodeHandle pnh("~");

    FwisNonlinearMpcControllerNode controller(nh, pnh);
