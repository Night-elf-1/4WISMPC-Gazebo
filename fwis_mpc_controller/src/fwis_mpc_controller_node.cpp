...
class FwisMpcControllerNode
{
public:
    FwisMpcControllerNode(ros::NodeHandle& nh, ros::NodeHandle& pnh)
          ...
          tf_listener_(tf_buffer_)
    {
        ...
        pnh.param<double>("front_wheelbase", param_.L_f, 0.4615);
        pnh.param<double>("rear_wheelbase",  param_.L_r, 0.4725);
        // 该 Gazebo 模型四个轮子的转动轴方向相同（URDF 中 axis 均为 +y），
        // 前进时四个轮子的角速度指令应为同号。若你的模型后轮关节方向相反，
        // 可通过此参数取反。
        pnh.param<double>("rear_wheel_velocity_sign", rear_wheel_velocity_sign_, 1.0);

        // 订阅参考路径、曲率、速度和里程计
        path_sub_ = nh.subscribe("/reference_path", 1, &FwisMpcControllerNode::pathCallback, this);
        curvature_sub_ = nh.subscribe("/reference_curvature", 1, &FwisMpcControllerNode::curvatureCallback, this);
        speed_sub_ = nh.subscribe("/reference_speeds", 1, &FwisMpcControllerNode::speedCallback, this);
        odom_sub_ = nh.subscribe("/odom", 1, &FwisMpcControllerNode::odomCallback, this);
        ...
        // 调试：发布最近参考点
        nearest_point_pub_ = nh.advertise<visualization_msgs::Marker>("/mpc_nearest_point", 1);

        // 控制定时器
        timer_ = nh.createTimer(ros::Duration(1.0 / control_rate_), &FwisMpcControllerNode::controlLoop, this);

        ROS_INFO("FWIS MPC controller initialized. wheel_radius=%.3f, control_rate=%.1f Hz",
...
private:
    ...
    void controlLoop(const ros::TimerEvent&)
    {
        ...
        if(min_e > 1.0)
        {
            ...
            ROS_WARN_THROTTLE(1.0, "Lateral error %.3f, resetting MPC control to reference.", min_e);
        }
        ...
        try
        {
            ...
            U_solve = mpc_.mpc_solve(r_x_, r_y_, ryaw_, rcurvature_, speed_profile_, initial_x, min_index, min_e, param_);
        ...
        msg.data = rear_wheel_velocity_sign_ * U_solve(1) / wheel_radius_;
        pub_whell_rear_L_.publish(msg);

        msg.data = rear_wheel_velocity_sign_ * U_solve(2) / wheel_radius_;
        ...
        ROS_INFO_THROTTLE(1.0, "MPC: nearest_idx=%d, nearest_dist=%.3f, solve_time=%.2fms, v=[%.2f, %.2f, %.2f, %.2f]",
                          min_index, min_e, solve_time_ms,
                          U_solve(0), U_solve(1), U_solve(2), U_solve(3));
        ROS_INFO_THROTTLE(1.0, "MPC: steering=[%.3f, %.3f, %.3f, %.3f], v_body=%.3f, omega=%.3f",
    ...
    void resetControlToReference(int min_index)
    {
        ...
        double k_r = std::max(-2.5, std::min(2.5, rcurvature_[min_index]));
        double omega_r = v_r * k_r;
        double L_f = param_.L_f;
        double L_r = param_.L_r;
        double W = param_.W;
        double xw[4] = { L_f, -L_r, -L_r, L_f };
        double yw[4] = {   W,    W,   -W,  -W };
...
private:
    ...
    double rear_wheel_velocity_sign_ = 1.0;
};

int main(int argc, char** argv)
{
    ros::init(argc, argv, "fwis_mpc_controller_node");
    ros::NodeHandle nh;
    ros::NodeHandle pnh("~");

    FwisMpcControllerNode controller(nh, pnh);

    // 使用多线程 spinner，避免 MPC 求解耗时阻塞订阅回调
