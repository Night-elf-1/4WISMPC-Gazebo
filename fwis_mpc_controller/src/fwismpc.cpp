...
Eigen::VectorXd fwisMpcController::mpc_solve(vector<double>& cx, vector<double>& cy, vector<double>& cyaw, vector<double>& ck, vector<double>& speed, Eigen::Vector3d inital_x, int min_index, double min_errors, parameters params_){
    ...
    // 控制量物理约束 (4个轮速，4个转角，车体纵向速度 v_body，横摆角速度 omega)
    Eigen::VectorXd u_min(NU), u_max(NU), delta_umin(NU), delta_umax(NU);
    u_min << -3.0, -3.0, -3.0, -3.0, -1.5, -1.5, -1.5, -1.5, -3.0, -5.0;
    u_max <<  3.0,  3.0,  3.0,  3.0,  1.5,  1.5,  1.5,  1.5,  3.0,  5.0;
    delta_umin << -0.5, -0.5, -0.5, -0.5, -0.1, -0.1, -0.1, -0.1, -0.5, -0.2;
    delta_umax <<  0.5,  0.5,  0.5,  0.5,  0.1,  0.1,  0.1,  0.1,  0.5,  0.2;
    ...
    // 参考车体速度 / 横摆角速度，作为辅助变量用于 ICR 等式约束
    U_r(8) = v_r;
    U_r(9) = omega_r;
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // 与 car_model 几何一致的前后轮距
        double denom = 2.0 * (L_f * L_f + L_r * L_r + 2.0 * W * W);
        double Ji = (xw[i] * sin(delta_ir) - yw[i] * cos(delta_ir)) / denom * dt;
        double Ki = v_ir * (xw[i] * cos(delta_ir) + yw[i] * sin(delta_ir)) / denom * dt;
    ...
    // ==========================================
    const int n_ineq_rows = 2 * NC * NU;
    const int n_eq_rows   = 4;               // 只对当前控制步施加 4 个 ICR 转角等式约束
    const int n_cons_rows = n_ineq_rows + n_eq_rows;
    ...
    static Eigen::MatrixXd A_cons = Eigen::MatrixXd::Zero(n_cons_rows, NC * NU);
    
    if (!is_matrix_initialized) {
        Eigen::MatrixXd Q_step = Eigen::MatrixXd::Zero(NY, NY);
        Q_step.diagonal().head(NX) << 50.0, 50.0, 20.0;     
        // 控制量跟踪权重：轮速、转角、车体速度/横摆角速度
        Eigen::VectorXd q_tail(NU);
        q_tail << 50.0, 50.0, 50.0, 50.0, 10.0, 10.0, 10.0, 10.0, 1.0, 1.0;
        Q_step.diagonal().tail(NU) = q_tail;
        ...
        r_diag << 1.0, 1.0, 1.0, 1.0, 5.0, 5.0, 5.0, 5.0, 1.0, 1.0;
        R_step.diagonal() = r_diag;
        ...
        A_cons.middleRows(NC * NU, NC * NU) = A_e;   

        is_matrix_initialized = true;
    }

    // ==========================================
    // 4WIS ICR 转角耦合等式约束（在参考点线性化）
    // F_i = (v - yw_i*omega)*sin(delta_i) - xw_i*omega*cos(delta_i) = 0
    // 强制当前控制步的四个转角指向同一个瞬时转动中心
    // ==========================================
    Eigen::VectorXd eq_rhs = Eigen::VectorXd::Zero(n_eq_rows);
    for (int i = 0; i < 4; ++i) {
        int row = n_ineq_rows + i;

        double delta_ir = U_r(i + 4);
        double c_v  = std::sin(delta_ir);
        double c_w  = -yw[i] * std::sin(delta_ir) - xw[i] * std::cos(delta_ir);
        double c_d  = U_r(i);   // = vx_ir*cos(delta_ir) + vy_ir*sin(delta_ir)

        // 只约束即将下发的控制量：U_next = U + delta_U
        A_cons(row, i + 4) = c_d;
        A_cons(row, 8)     = c_v;
        A_cons(row, 9)     = c_w;

        double dev_delta = U(i + 4) - delta_ir;
        double dev_v     = U(8) - v_r;
        double dev_w     = U(9) - omega_r;
        eq_rhs(i) = -(c_d * dev_delta + c_v * dev_v + c_w * dev_w);
    }
    ...
    Eigen::VectorXd lowerBound(n_cons_rows);
    Eigen::VectorXd upperBound(n_cons_rows);
    
    lowerBound.head(NC * NU) = delta_Umin_seq;
    lowerBound.segment(NC * NU, NC * NU) = Umin_seq - U_rep;
    lowerBound.tail(n_eq_rows) = eq_rhs;
    
    upperBound.head(NC * NU) = delta_Umax_seq;
    upperBound.segment(NC * NU, NC * NU) = Umax_seq - U_rep;
    upperBound.tail(n_eq_rows) = eq_rhs;
